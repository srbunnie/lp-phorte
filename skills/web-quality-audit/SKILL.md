...
1. Prefer a real Lighthouse audit through the configured MCP server
2. Reuse saved Lighthouse JSON reports from `lighthouse auditories/` when they already exist
3. Fall back to static project analysis when a live audit is not available
4. Categorize findings by severity (Critical, High, Medium, Low)
5. Provide specific, actionable recommendations
6. Include code examples for fixes

## Gemini CLI and MCP

In this repository, Gemini CLI should read project MCP settings from `.gemini/settings.json`.
...
```json
{
  "mcpServers": {
    "lighthouse": {
      "command": "npx",
      "args": ["-y", "@danielsogl/lighthouse-mcp@1.2.17"],
      "timeout": 120000
    }
  }
}
...
Use the following audit sources in this order:

1. Live Lighthouse MCP run against the target URL
2. Saved JSON reports in `lighthouse auditories/`
3. Static code/project inspection

For saved reports, prefer the Node helper:

```bash
node .agents/skills/web-quality-audit/scripts/summarize-lighthouse-report.mjs "lighthouse auditories/lighouse marcelo vignola site.txt"
```

## Project notes

This project runs on Windows and uses Next.js App Router. Prefer Node-based helpers over Bash-only scripts for repeatable audit workflows.
